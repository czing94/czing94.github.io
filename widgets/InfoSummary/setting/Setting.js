        ...
        for (var i = 0; i < this.config.layerInfos.length; i++) {
          ...
          if (!jimuLayerInfo) {
            ...
            for (var ii = 0; ii < this.opLayers._layerInfos.length; ii++) {
              ...
              if (originOpLayer && originOpLayer.url && configLayer.url &&
              ...
              } else if ((_opLayer && _opLayer.subId && _opLayer.subId === configLayer.id) ||
      ...
      _setLayers: function () {
        ...
        for (var i = 0; i < this.opLayers._layerInfos.length; i++) {
          ...
          if (OpLyr.originOperLayer) {
            ...
            lyrType = originOpLayer.layerType;
          ...
          var newSubLayers = (lyrType === 'GeoRSS' || lyrType === 'KML') ? lyrInfo.newSubLayers : OpLyr.newSubLayers;
          if (newSubLayers.length > 0) {
            var hasNested = this.checkNestedGroups(newSubLayers);
            if (!hasNested || lyrType === 'KML') {
              var subLayers = newSubLayers;
              var parentID;
